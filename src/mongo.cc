...
public:
    ...
    if(!expectedlen)
      {
        ...
        bson_little_endian32(&expectedlen, &reply_head.len);
      }

    if (expectedlen-buflen == 0) {
  ...
  void ParseReply(mongo_header reply_head, mongo_reply_fields reply_fields) {
    ...
    if ( (!cursor->mm || ! reply_fields.cursorID) || reply_fields.num == limit)
      {
        pdebug("finished, forwarding results\n");
        FreeCursor();
        get_more = false;
        EmitResults();
        results.Dispose();
  ...
  void ConsumeInput() {
    ...
    for (;;) {
      ...
      pdebug("reading %d bytes current: %d expected: %d\n", readbuflen, buflen, expectedlen);
